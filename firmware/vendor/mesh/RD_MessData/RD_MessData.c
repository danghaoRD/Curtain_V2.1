...
extern STT_Curtain_Mess STT_Curtain_Mess_Config;
...
RD_TYPE_TOUCH_SWITCH			TYPE_TOUCH_SWITCH	= RD_TYPE_CURTAIN;
#endif
...
int RD_Messenger_ProcessCommingProcess_SCENE(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par)
{
	...
	uint8_t  Control_Stt_Buff=0x00;
	Header_buff = ((par[1]<<8) | par[0]);
	Gw_Add_Buff = cb_par->adr_src;

	switch (Header_buff)
	{
		case RD_HEADER_CONTROL_CURTAIN:    // RD_EDIT CURTAIN Tested
		{
			Control_Stt_Buff = par[2];
			switch (Control_Stt_Buff)
			{
				 case 0x00:
				...
				sprintf(UART_TempSend,"Control Curtain: %d\n",Control_Stt_Val);
				...
				mesh_tx_cmd2normal_primary(RD_OPCODE_SCENE_RSP, RD_Messenger_TempSend, 3, Gw_Add_Buff, RD_MAXRESPONESEND);
			#if UART_ON
				uart_CSend("Responed");
			#endif
		}
		break;
		case RD_HEADER_CURTAIN_STT: // RD_EDIT CURTAIN Test
		{
			uint8_t Open_Percent_Buff;
			Open_Percent_Buff = (Time_Control_State_Now*100)/Time_Control_Max ;
			Send_Relay_Stt_Message(Control_Stt_Val, Open_Percent_Buff);
		}
			break;
		case RD_HEADER_CURTAIN_SAVE_SCENE: // RD_EDIT CURTAIN Test
		{
...
//			Stt_Relay_Buff[3] = par[7];
			...
			Check_buff = RD_Flash_SaveScene(Scene_ID_Buff, Open_Percent_Buff);  // save Scene to Flash  Open_Percent_Buff is % Open Now
			...
			Save_Scene_Mess_Config.Header[0]    = RD_HEADER_CURTAIN_SAVE_SCENE & 0xff;
			Save_Scene_Mess_Config.Header[1]	= RD_HEADER_CURTAIN_SAVE_SCENE >>8;
			Save_Scene_Mess_Config.Position_Saved = Open_Percent_Buff;  // % Open
			Save_Scene_Mess_Config.Future[0] = par[5];
			Save_Scene_Mess_Config.Future[1] = par[6];
			Save_Scene_Mess_Config.Future[2] = par[7];
			...
			break;
		case RD_HEADER_CURTAIN_DELETE_SCENE:
		{
			uint16_t Scene_ID_Buff = 0x0000;
			...
			Delete_Scene_Mess_Config.Header[0] = RD_HEADER_CURTAIN_DELETE_SCENE & 0xff;
			Delete_Scene_Mess_Config.Header[1] = RD_HEADER_CURTAIN_DELETE_SCENE >> 8;
			...
			Save_Scene_Mess_Config.Position_Saved = 0x00;  // % Open
			Save_Scene_Mess_Config.Future[0] = 0x00;
			Save_Scene_Mess_Config.Future[1] = 0x00;
			Save_Scene_Mess_Config.Future[2] = 0x00;
			...
			break;
			//RD_EDIT: test new calib
		case RD_HEADER_CURTAIN_CALIB:
		{
			calib_Stt=1;
			...
			switch (Control_Stt_Buff)
			{
				...
				#if UART_ON
					char UART_TempSend[128];
					sprintf(UART_TempSend,"Control Curtain Calib: %d\n",Control_Stt_Val);
					uart_CSend(UART_TempSend);
				#endif
					uint8_t *RD_Messenger_TempSend;
					RD_Messenger_TempSend   = par;
...
int RD_Messenger_ProcessCommingProcess_Type(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par){
	...
	switch (header_Buff)
	{
		case(RD_HEADER_RSP_TYPE_DEVICE):		// RD_EDIT: CURTAIN Test
		{
...
void Send_Relay_Stt_Message(uint8_t Control_Taget,uint8_t Open_percent)
{ //RD_EDIT CURTAIN Test

	STT_Curtain_Mess_Config.Header[0] 	= RD_HEADER_CURTAIN_STT & 0xFF ;
	STT_Curtain_Mess_Config.Header[1] 	= RD_HEADER_CURTAIN_STT >>8;
	STT_Curtain_Mess_Config.Control_Taget	= (uint8_t) Control_Taget;
	STT_Curtain_Mess_Config.CurtainStt 	= Open_percent;
	STT_Curtain_Mess_Config.Future[0] 	= 0x00;
	STT_Curtain_Mess_Config.Future[1] 	= 0x00;
	STT_Curtain_Mess_Config.Future[2]	= 0x00;
	STT_Curtain_Mess_Config.Future[3]	= 0x00;

	uint8_t* Mess_Buff;
	Mess_Buff = (uint8_t *) (&STT_Curtain_Mess_Config);
	mesh_tx_cmd2normal_primary(RD_OPCODE_SCENE_RSP, Mess_Buff, 4, RD_GATEWAYADDRESS, RD_MAXRESPONESEND); //SENSOR_STATUS
}
