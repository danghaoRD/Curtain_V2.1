...
void RD_Flash_Init()
{
	...
	else
	{
		...
		sprintf(UART_TempSend,"Flash data 0x780000:0x%x  \n GW add: 0x%x   \n  ",Sw_Flash_Data_Val.Factory_Check, RD_GATEWAYADDRESS);
	...
	else
	{
		...
		*/
		Counter_Flash_Power_Eraser = 	(Sw_Flash_Data_Save_Power_Off_Val.Counter_Eraser[0] << 24) | (Sw_Flash_Data_Save_Power_Off_Val.Counter_Eraser[1] << 16) | \
									 	(Sw_Flash_Data_Save_Power_Off_Val.Counter_Eraser[2] << 8)  | (Sw_Flash_Data_Save_Power_Off_Val.Counter_Eraser[3]);
		char UART_TempSend[128];
		Time_Control_State_Now = (Sw_Flash_Data_Save_Power_Off_Val.OpenStt*Time_Control_Max)/100;  //
		sprintf(UART_TempSend,"Flash POWER Off: Add: 0x%x---- Count Eraser:0x%x  \n  \
								Pre Open Stt= %d % Open",(DATA_POWER_OFF_FLASH_ADDR + (0x1000*Jump_Flash)), Counter_Flash_Power_Eraser   \
				   	   	   	   	,Sw_Flash_Data_Save_Power_Off_Val.OpenStt   );
		uart_CSend(UART_TempSend);
	}
...
int RD_Flash_SaveScene(uint16_t Scene_ID, uint8_t Position_Save)
{
	uint8_t *Flash_Data_Buff;
	Flash_Data_Buff  = (uint8_t *) (&Sw_Flash_Data_Val);
	...
	else
	{
		...
		*(Flash_Data_Buff+Scene_Saved+2) = Position_Save;
	   	flash_erase_sector(FLASH_ADDR);
		flash_write_page(FLASH_ADDR, FLASH_DATA_SIZE,Flash_Data_Buff );
	#if UART_ON
		char UART_TempSend[128];
		sprintf(UART_TempSend,"Saved Scene: %d\n", ((Scene_Saved-5)/6+1));
		uart_CSend(UART_TempSend);
		return 1;  // success
	#endif
	}
...
//	#endif
	...
	else
	{
		...
		uint8_t Position_Buff = *(Flash_Data_Buff+Scene_Saved+2);
		  RD_Curtains_Goto(Position_Buff);

	#if UART_ON
		char UART_TempSend[128];
		sprintf(UART_TempSend,"Active Scene:0x%x  goto Position:%d %Open n",Scene_ID,  Position_Buff);
		uart_CSend(UART_TempSend);
	#endif
	}
...
void RD_Flash_SaveOpenStt(uint8_t OpenPercent)
{
	Sw_Flash_Data_Save_Power_Off_Val.OpenStt	= OpenPercent;
	...
	#if UART_ON
		char UART_TempSend[128];
		sprintf(UART_TempSend," \n Eraser: %d Add: 0x%x  Saved Open Stt: %d %  Open\n",  Counter_Flash_Power_Eraser, (DATA_POWER_OFF_FLASH_ADDR + (0x1000*Jump_Flash)), 	Sw_Flash_Data_Save_Power_Off_Val.OpenStt  );
		uart_CSend(UART_TempSend);
	#endif
}
